...

{
...

    {
...

        {
            ...
            logService.Log(LogLevel.Info, "Ensuring a clean slate for Winhance Power Plan...");

            // FORCE DELETE (ignore failure if it doesn't exist)
            // This clears any "ghost" GUIDs that are blocking the duplication
            await RunPowercfgAsync($"/delete {predefinedPlan.Guid}").ConfigureAwait(false);
